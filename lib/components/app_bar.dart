...
class _MyAppbarState extends State<MyAppbar> {
  ...
  Widget build(BuildContext context) {
    return  AppBar(title: Padding(
      ...
      child: Row(
        children: [
          Image.asset("assets/AmberIcon.ico",height: 50,width: 50,),
          const Text("AMBER",style: TextStyle(fontWeight: FontWeight.normal,fontSize: 30),)
        ],
      ),
    ),
      ...
      actions: [
        Padding(
          ...
          child: PopupMenuButton<int>(color: Colors.white,
              ...
              [
                ...
                const PopupMenuItem<int>(
                    value: 1,
                    child: Text("Quotation Report")),
                const PopupMenuItem<int>(
                    value: 2,
                    child: Text("Categorywise Profit")),
                const PopupMenuItem<int>(
                    value: 3,
                ...
                const PopupMenuItem<int>(
                    value: 5,
                    child: Text("Purchase Comparison")),
                const PopupMenuItem<int>(
                    value: 6,
                    child: Text("Salesorder Report")),
                const PopupMenuItem<int>(
                    value: 7,
                    child: Text("Cheque Report")),
            ...
            if(menu == 1){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const QuotationReport()));
            }

            if(menu == 2){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const CategoryProfit()));
            }
            ...
            if(menu == 4){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const SalesComparison()));
            }

            if(menu == 5){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const PurchaseComparison()));
            }

            if(menu == 6){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const SalesorderReport()));
            }

            if(menu == 7){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>const ChequeNotificationReceived()));
            }
        ...
        Padding(
          padding: const EdgeInsets.only(right: 10,top: 10),
          child: GestureDetector(onTap: ()=> _showMyDialog(),child: const Icon(Icons.logout),),
        )
      ],);
  }
            ...
            TextButton(
              ...
              onPressed: () async {
               ...
               //Navigator.pushReplacement(context, MaterialPageRoute(builder: (BuildContext context) => Login_page()));
                Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (ctx) => const Login_page()),(route) => false);
              },
            ),
            TextButton(
